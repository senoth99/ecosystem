...
export async function getTelegramAllowanceRole(tgUser: TgMiniAppUser): Promise<UserRoleValue> {
  ...
  if (fallbackAdminAllowed) return UserRole.SUPER_ADMIN;
  if (allowed?.role === UserRole.ADMIN) return UserRole.ADMIN;
  if (allowed?.role === UserRole.SUPER_ADMIN) return UserRole.SUPER_ADMIN;

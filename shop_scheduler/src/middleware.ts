...
export async function middleware(req: NextRequest): Promise<NextResponse> {
  ...
  requestHeaders.set("x-eco-superadmin", session.isSuperAdmin ? "1" : "0");

  if (MULTI_ZONE_ENABLED && !pathname.startsWith("/select-point") && pathname !== "/welcome") {
    // zone selection unchanged — handled in pages
  }
